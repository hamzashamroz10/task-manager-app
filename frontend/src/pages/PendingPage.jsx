            ...
            className='px-3 py-2 border border-purple-100 rounded-lg focus:ring-2 focus:ring-purple-500 md:hidden text-sm'>
             ...
             <div className="hidden md:flex space-x-1 bg-purple-50 p-1 rounded-lg ml-3">
              {SORT_OPTIONS.map(opt => (
                ...
                className={tab.tabButton(sortBy === opt.id)}>
                  {opt.icon}{opt.label}
                </button>
              ))}
             </div>
    ...
    <div className='p-8 bg-white rounded-xl shadow-sm border border-purple-100 text-center'>
      <div className=' max-w-xs mx-auto py-6'>
        ...
        <button onClick={() => setShowModal(true)}
          className='px-4 py-2 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded-lg text-sm font-medium transition-colors'>Create New Task</button>
      </div>
    </div>
  ) : (sortedPendingTasks.map(task => (
  <TaskItem key={task._id || task.id} task={task} showCompleteCheckbox onDelete={() => handleDelete(task._id || task.id)} onToggleComplete = {() => handleToggleComplete(task._id || task.id, t.completed)}
