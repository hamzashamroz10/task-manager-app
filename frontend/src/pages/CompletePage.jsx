    ...
    <div className="flex items-center justify-between bg-white p-2 md:p-3 rounded-xl shadow-sm border border-purple-100 w-full md:w-auto">
        ...
        <div className="hidden md:flex space-x-1 bg-purple-50 p-1 rounded-lg ml-2 md:ml-3">
          {SORT_OPTIONS.map(opt => (
            ...
            className={['px-2 py-1 md:px-3 md:py-1.5 rounded-lg text-xs font-medium transition-all duration-200 flex items-center gap-1', sortBy === opt.id ? 'bg-white text-purple-700 shadow-sm border border-purple-100' : 'text-gray-600 hover:text-purple-700 hover:bg-purple-100/50'].join(" ")}>
              {opt.icon}
              {opt.label}
            </button>
          ))}
