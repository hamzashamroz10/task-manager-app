...
const Layout = ({ user, onLogout }) => {
    ...
    <div className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-purple-100 hover:shadow-md transition-all duration-300 hover:border-purple-100 group">
      <div className="flex items-center gap-2">
        ...
        <div className='min-w-0'>
          <p className="text-lg sm:text-xl font-bold bg-gradient-to-r from-fuchsia-500 to-purple-600 bg-clip-text text-transparent">{value}</p>
  ...
  if (error) return (
    <div className='min-h-screen bg-gray-50 p-6 flex items-center justify-center'>
      <div className=' bg-red-50 text-red-600 p-4 rounded-xl border border-s-red-100 max-w-md'>
          ...
          <div className='xl:col-span-1 space-y-4 sm:space-y-6'>
            <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
              ...
              <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6'>
                ...
                <StatCard title='Pending Task' value={stats.pendingCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-fuchsia-500' />} />
              ...
              <div className='space-y-2 sm:space-y-3'>
                <div className='flex items-center justify-between text-gray-700'>
                  <span className='text-xs sm:text-sm font-medium flex items-center gap-1.5'>
                    <Circle className='w-2.5 h-2.5 sm:w-3 sm:h-3 text-purple-700 fill-purple-500' />
                    Task Progress
                  </span>
            ...
            <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
              <h3 className='text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-2'>
                <Clock className='w-4 h-4 sm:h-5 text-purple-500' />
                Recent Activity
              </h3>
              <div className='space-y-2 sm:space-y-3'>
                  ...
                  <div key={task._id || task.id} className="flex items-center justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-purple-100">
                    <div className='flex-1 min-w-0'>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {task.createAt ? new Date(task.createAt).toLocaleDateString() : "No Date"}
                      </p>
                    </div>
                    <span className={`px-2 text-xs rounded-full shrink-0 ml-2 ${task.completed ? 'bg-green-100 text-green-700' : 'bg-fuchsia-100 text-fuchsia-700'}`}>
                ...
                {tasks.length === 0 && (
                  <div className="text-center py-4 sm:py-6 px-2">
                    <div className="w-12 h-12 sm:h-16 sm:w-12 mx-auto sm:mb-4 rounded-full bg-purple-100 flex items-center justify-center">
